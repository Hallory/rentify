    ...
    def validate(self, attrs):
        ...
        if not rental_property.is_rent():
            raise serializers.ValidationError(
                "Only properties with 'rent' deal type can be booked."
            )

        if not rental_property.is_published():
            raise serializers.ValidationError(
                "Only published properties can be booked."
            )
        ...
        if guests > rental_property.guests:
            raise serializers.ValidationError(
                "Number of guests cannot exceed property's maximum guests."
            )
        ...
        if overlapping_bookings.exists():
            raise serializers.ValidationError(
                "Property is already booked for the specified dates."
            )
